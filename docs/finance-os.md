...
| **PayPal** | Aktivitäten → download → Benutzerdefiniert → "Guthaben-relevant" → CSV | 7y, in 12-month chunks | Real merchant in `Name`; `Brutto/Entgelt/Netto` separate; `Typ` may not reliably flag recurring *(unverified)* — treat recurring detection as heuristic. "Automatische Zahlungen" wallet view is view-only, not exportable |
